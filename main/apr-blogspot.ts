// четверг, 29 мая 2025 г. 08:27:05 (MSK)
...
class OLY implements IOLY {
	...
	/**
	 * Метод возвращает название дня на русском языке
	 *
	 * @returns String
	 */
	ruday() {
	...
	initDatesOLY(): {} {
		...
		// this.datesOLY['dopip'] = [
		//     Math.ceil((this.datesOLY.pentecost[0] - this.datesOLY.pip[0]) / 864e5),
		//     'Пятьдесятница',
		// ]
	...
	controlDates(userYear: [number, number?, number?] | undefined): Date {
		...
		if (currentDate != this.theMomentTime) {
			...
			document.querySelector(
				'body'
			)!.innerHTML += `<div id="userdate" class='userdate'><a id='a-visited-userdate' href="#" onclick="apr.deleteUserDateFromSessionStorage()">${currentDate.toLocaleDateString()}</a></div>`
		}
		///////////////////////////////////////////////////////////////////////////////////////
		////////////////////////////////// end ////////////////////////////////////////////////
		///////////////////////////////////////////////////////////////////////////////////////
	...
	info() {
		...
		console.info(`
            ...
            Ссылка на праздник: https://aprakos.blogspot.com${
							this.linkToHolydays ?? ''
						}
	...
	modalCweek50() {
		...
		return `По Пять&shy;десят&shy;нице <span class="red bold">${
			this.weeks.current[0] > 7 ? Number(this.anchorElemID) - 7 : 'нет'
		}</span>`
	}
	...
	initModalView() {
		...
		const closeClick =
			'<span id="close" class="close" onclick="apr.closeModalView()"></span>'
		const commentStvol =
			"<span class='comment-stvol'><br> Подробнее<a class='a-href' href='https://www.aprakos.ru/p/blog-page.html'> здесь</a>.</div>"

		let str = `
        <section id="fp-content" class="fp-content">
        <b>Седмица Евангелия: </b>
        <div id="modal-cweek">По Пасхе&nbsp; <span style="background-color: white; border-radius: 50%; padding: 0 .7rem 0" class="red bold">${
					this.anchorElemID
				}</span></div>
        ${this.modalCweek50()}
        <div>${
        ...
        <div>${
					lastSegment === 'stvol.html'
						? `${this.weeks.stupkaK[1]} <span class="red bold">${Math.abs(
								this.weeks.stupkaK[0]
						  )}</span> седм.`
						: ''
				}</div></div>
        <div>${
					lastSegment === 'stvol.html'
						? `${this.weeks.all[1]} <span class="red bold">${Math.abs(
								this.weeks.all[0]
						  )}</span> седм.`
						: ''
				}</div></div>
	...
	correctorStupka() {
		this.weeks.stupkaV[1] = String(
			this.weeks.stupkaV[0] <= 0
	...
	initElementsDOM() {
		...
		document
			...
			?.children[0].setAttribute(
				'href',
				this.linkToHolydays ?? this.linkToAprakos
			)
		...
		let elemsID: { [key: string]: string } = {
			...
			curweek50: `${
				this.weeks.current[0] < 8 ? '*' : this.weeks.current[0] - 7
			}`,
			...
			if (Object.prototype.hasOwnProperty.call(elemsID, atrubuteID)) {
				if (atrubuteID === 'curweek') {
					document.getElementById(
						atrubuteID
					)!.innerHTML = `<a href="#week${this.weeks.apstlElemID[0]}">${elemsID[atrubuteID]}</a>`
					// throw new ReferenceError(eid)
				} else if (atrubuteID === 'curweek50') {
					document.getElementById(
						atrubuteID
					)!.innerHTML = `<a href="#week${this.weeks.evnglElemID[0]}">${elemsID[atrubuteID]}</a>`
		...
		document.getElementById(
			'weekday' + this.weeks.apstlElemID[0] + this.weeks.day[0]
		)!.className += ' apstl-day'
		document.getElementById(
			'weekday' + this.weeks.apstlElemID[0] + this.weeks.day[0]
		...
		if (this.weeks.evnglElemID[0] != this.weeks.apstlElemID[0]) {
			document.getElementById(
				'weekday' + this.weeks.evnglElemID[0] + this.weeks.day[0]
			)!.className += ' evngl-day'
		...
		if (this.weeks.evnglElemID[0] == 50) {
			document
				...
				.setAttribute(
					'style',
					'border: solid 4rem #fedede; background-color: #fedede;'
				)
		}
	}
	...
	eventKeys() {
		...
		document.addEventListener('keyup', event => {
			// безымянку не можно заглушить методом `removeAddEventListener`
			if (event.key == 'F2') {
...
class SelectedDay {
	...
	setColor() {
		...
		if (this.userDate_ss) {
			...
			let dateFromForm = document.querySelector(
				'input[type="date"]'
			...
			!this.checkbox
				? (dateFromForm.value = apr.theMomentOffsetZone
						.toISOString()
						.slice(0, 10))
				: (dateFromForm.value = sessionStorage.getItem(
						'lastInstalledDate'
				  ) as string)
